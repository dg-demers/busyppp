...
# Busyppp takes one argument which should be 
#   a list of caret (^)-separated URLs of files to download with the whole list enclosed in single-quotes ('), for example, 
#     $ busyppp.sh 'https://somewhere.de/somepdf.pdf ^ http://someplace.au/apackage.deb'
#   OR 
#   the name of a file (in the current directory/folder) which contains a list of URLs for the files to download, 
...
# A related default value is 0.40 kiB/s in the script line below
#   if (($(bc <<< "$brate < 0.40"))); 
